...
/**
 * StartAgentServiceHandlerImpl
 * 
 * @author Nicolas Moser, PRODYNA AG
 */
public class StartAgentServiceHandlerImpl extends StartAgentServiceHandler {

    private static final long serialVersionUID = 1L;

    @Override
    protected EmptyServiceMessage startAgent(AgentNameRq msg) throws AgentException {

        if (msg.getAgentName() == null || msg.getAgentName().getValue() == null) {
            super.getLogger().warning("Cannot start agent with name [null].");
        ...
        if (agent == null) {
            super.getLogger().warning("Cannot start agent with name [", agentName, "].");
        ...
        if (agent.getActive().getValue()) {
            super.getLogger().info("Agent [", agentName, "] is already running.");
        } else {
            super.getLogger().info("Activating agent with name [", agentName, "].");
